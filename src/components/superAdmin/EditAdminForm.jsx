...
const validationSchema = Yup.object({
  ...
  username: Yup.string().required('Username is required').min(3).max(30).matches(/^[a-zA-Z0-9_]+$/),
  email: Yup.string().required('Email is required').email(),
  department: Yup.string().max(50),
  jobRole: Yup.string().max(50),
  password: Yup.string().min(8).matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/, 'Must contain uppercase, lowercase, number'),
});

const EditAdminForm = ({ open, onClose, admin, onSave, isLoading = false, error = null }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      Object.keys(values).forEach((key) => {
        if (values[key] !== '' && !(key === 'password' && values[key] === '')) {
  ...
  useEffect(() => {
    if (admin?.profileImage) {
      const url = admin.profileImage.startsWith('http') ? admin.profileImage : `${process.env.REACT_APP_API_BASE_URL}${admin.profileImage}`;
      setImagePreview(url);
    }
  }, [admin]);
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
      <DialogTitle>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Typography variant="h6" sx={{ fontWeight: 600 }}>Edit Admin</Typography>
      ...
      <form onSubmit={formik.handleSubmit}>
        <DialogContent dividers>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField fullWidth id="password" name="password" label="New Password (optional)" type="password" value={formik.values.password} onChange={formik.handleChange} onBlur={formik.handleBlur} error={formik.touched.password && Boolean(formik.errors.password)} helperText={formik.touched.password && formik.errors.password || 'Leave blank to keep current password'} disabled={isLoading} />
            </Grid>
            <Grid item xs={12} sm={6}>
